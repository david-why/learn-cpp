...
namespace Graph
{
    ...
    public:
        ...
        int dist(int i, int j, int l = -1)
        {
            if (l == -1)
                l = lca(i, j);
            return dep[i] + dep[j] - dep[l] - dep[l];
        }
    };
}
using MyIO::in;
...
int main()
{
    ...
    while (m--)
    {
        int x, y, z;
        in(x, y, z);
        int l1 = tr.lca(x, y), l2 = tr.lca(x, z), l3 = tr.lca(y, z);
        int d1 = tr.dist(x, y, l1), d2 = tr.dist(x, z, l2), d3 = tr.dist(y, z, l3);
        int e1 = tr.dist(l1, z), e2 = tr.dist(l2, y), e3 = tr.dist(l3, x);
        int mx = e1 + d1, pos = l1;
        if (e2 + d2 < mx)
            mx = e2 + d2, pos = l2;
        if (e3 + d3 < mx)
