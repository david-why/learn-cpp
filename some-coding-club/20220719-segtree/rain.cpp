...
{
    int y, r;
    operator Node() const { return Node{r, r == 0 ? 0 : 1}; }
};
istream &operator>>(istream &is, year &y) { return is >> y.y >> y.r; }
...
int main()
{
    ...
    {
        if (dat[i].y - dat[i - 1].y > 1)
            n++;
        a[i] = yrs[dat[i].y] = ++n;
    }

    auto st = SegTree::make<Node>(n, [](Node a, Node b)
    ...
    while (m--)
    {
        ...
        if (x <= y)
            ret(1 false);
            cout<<"OK"<<endl;
        if (yrs.find(x) == yrs.end() || yrs.find(y) == yrs.end())
            ret(2 false);
            cout<<"OK"<<endl;
        int xa = yrs[x], ya = yrs[y];
            cout<<"OK "<<xa<<" "<<ya<<" "<<x<<" "<<y<<endl;
        int xmx = st.query(xa).mx;
            cout<<"OK"<<endl;
        if (xmx > st.query(ya).mx)
            ret(3 false);
            cout<<"OK"<<endl;
        if (ya + 1 <= xa - 1 && xmx <= st.query(ya + 1, xa - 1).mx)
            ret(4 false);
            cout<<"OK"<<endl;
        Node nd = st.query(xa, ya);
        if (nd.cnt != ya - xa + 1)
            ret(5 maybe);
            cout<<"OK"<<endl;
        ret(6 true);
