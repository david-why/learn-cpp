// https://www.luogu.com.cn/problem/P1462?contestId=86167
...
int dfs(int c, int b, int a)
{
    ...
    int ans = 1e9;
    v[c] = true;
    for (pair<const int, int> &p : con[c])
    {
        if (v[p.first] || b < p.second)
            continue;
        ans = min(ans, dfs(p.first, b - p.second, a));
    }
    v[c] = false;
    return ans;
}
