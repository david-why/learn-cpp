...
#define frj(n) for (int j = 1; j <= n; j++)
...
const int maxn = 20007, maxm = 1e5 + 7;

int n, m;
vector<pii> row[maxn], col[maxn];
unordered_map<int, int> con[maxm * 2];
...
int main()
{
    ...
    fri(m + 2)
    {
        ...
        row[x].pb(mp(y, i));
        col[y].pb(mp(x, i));
        con[i][i + m + 2] = con[i + m + 2][i] = 1;
    }
    fri(n) sort(bte(row[i])), sort(bte(col[i]));
    fri(n) fj0(((int)row[i].size()) - 1)
        con[row[i][j].second][row[i][j + 1].second] =
            con[row[i][j + 1].second][row[i][j].second] =
                2 * (row[i][j + 1].first - row[i][j].first);
    fri(n) fj0(((int)col[i].size()) - 1)
        con[col[i][j].second + m + 2][col[i][j + 1].second + m + 2] =
            con[col[i][j + 1].second + m + 2][col[i][j].second + m + 2] =
                2 * (col[i][j + 1].first - col[i][j].first);
    con[m + 2 + m + 3][m + 1] = con[m + 2 + m + 3][m + 1 + m + 2] = 0;
    con[m + 2][m + 2 + m + 4] = con[m + 2 + m + 2][m + 2 + m + 4] = 0;
    out - dijkstra(m + 2 + m + 3, m + 2 + m + 4) - nl;
