...
namespace mfs
{
    ...
    public:
        mfs(int n = maxn - 1) : n(n) { init(); }
        void init()
        {
            for (int i = 0; i <= n; i++)
                par[i] = i, cnt[i] = 1;
        }
        void resize(int n) { this->n = n, init(); }
        int size() { return n; }
        ...
        int count(int i) { return cnt[find(i)]; }
    };
}
namespace modular
{
...
int m, n;
int con[maxn][maxn];
mfs::mfs<maxn * maxn> gr;

#define idx(i, j) ((50) * (i) + (j))

int calc(int i, int j)
{
    if (gr.find(i) == gr.find(j))
        return gr.count(i);
    return gr.count(i) + gr.count(j);
}
...
int main()
{
    ...
    fi0(n) fj0(m)
    {
        in - con[i][j];
        if (!(con[i][j] & 1))
            gr.merge(idx(i, j - 1), idx(i, j));
        if (!(con[i][j] & 2))
            gr.merge(idx(i - 1, j), idx(i, j));
    }
    int mx = 0, cnt = 0;
    fi0(n) fj0(m)
    {
        int x = idx(i, j);
        if (gr.find(x) == x)
            mx = max(mx, gr.count(x)), cnt++;
    }
    out(cnt, nl, mx, nl);
    mx = 0;
    int mi = 0, mj = 0; // ms=2N/4E
    char ms = 0;
    fj0(m) for (int i = n - 1; i >= 0; i--)
    {
        if ((con[i][j] & 2) && i > 0)
        {
            int v = calc(idx(i, j), idx(i - 1, j));
            if (v > mx)
                mx = v, mi = i, mj = j, ms = 'N';
        }
        if ((con[i][j] & 4) && j < m - 1)
        {
            int v = calc(idx(i, j), idx(i, j + 1));
            if (v > mx)
                mx = v, mi = i, mj = j, ms = 'E';
        }
    }
    out - mx - nl - mt(mi + 1, mj + 1, ms) - nl;
