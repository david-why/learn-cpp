...
{
    double a, b;
    line(double x1, double y1, double x2, double y2) : a((x1 * y2 - x2 * y1) / (x1 * x2 * x2 - x1 * x1 * x2)), b((x2 * x2 * y1 - x1 * x1 * y2) / (x1 * x2 * x2 - x1 * x1 * x2)) {}
    bool on(double x, double y) { return eq(a * x * x + b * x - y, 0); }
};
struct
{
    double x, y;
...
int main()
{
    ...
    {
        inint(n, m);
        fi0(n) in(pigs[i].x, pigs[i].y);
        fi0(n) for (int j = i + 1; j < n; j++)
        {
            line li(pigs[i].x, pigs[i].y, pigs[j].x, pigs[j].y);
            if (li.a >= 0 || eq(pigs[i].x, pigs[j].x) || eq(pigs[i].x, 0) || eq(pigs[j].x, 0))
            {
                kil[i][j] = 1 << i;
                continue;
            }
        ...
        for (int s = 0; s < (1 << n); s++)
        {
            fi0(n) if (!(s & (1 << i)))
            {
                ...
                for (int j = i + 1; j < n; j++)
                    if (!(s & (1 << j)))
                        dp[s | kil[i][j]] = min(dp[s | kil[i][j]], dp[s] + 1);
            }
        }
        // fi0(1<<n) out-dp[i]-" ";out-nl;
        out - dp[(1 << n) - 1] - nl;
    }
