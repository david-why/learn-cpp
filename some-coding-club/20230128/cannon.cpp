...
int main()
{
    ...
    fri(n) {
        string l = in;
        fj0(m) res[i] |= (l[j] == 'P' ? 1 : 0) << j;
    }
    fi0((1 << m)) if (!((i & (i >> 2)) || (i & (i >> 1)))) nc.pb(i);    
    fri(n)
        for (int s : nc)
            if ((s & res[i]) == s)
                for (int p1 : nc)
                    if (!(s & p1))
                        for (int p2 : nc)
                            if (!(s & p2))
                                dp[i%2][p1][s] = max(dp[i%2][p1][s], dp[(i+1)%2][p2][p1] + __builtin_popcount(s));

    int ans = 0;
    for (int s : nc) for (int t : nc) ans = max(ans, dp[n%2][s][t]);
    out-ans-nl;
