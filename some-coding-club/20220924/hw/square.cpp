// https://www.luogu.com.cn/problem/P7074?contestId=84729
// Luogu P7074 [CSP-J2020] 方格取数
#include <bits/stdc++.h>
using namespace std;
...
{
    int n = in, m = in;
    vector<vector<int>> a(1, vector<int>(m + 1));
    a.reserve(n + 1);
    for (int i = 1; i <= n; i++)
        a.pb(in.vi(m)), a.back().insert(a.back().begin(), 0);
    vector<vector<vector<int>>> dp(m + 2, vector<vector<int>>(n + 2, vector<int>(2, -1e9)));
    ...
    for (int i = 1; i <= m; i++)
    {
        for (int j = 1; j <= n; j++)
        {
            ...
            for (int k = 0; k < 2; k++)
            {
                dp[i][j][k] = max(dp[i - 1][j][0], dp[i - 1][j][1]) + a[j][i];
                if (k && j != 1)
                    dp[i][j][k] = max(dp[i][j][k], dp[i][j - 1][k] + a[j][i]);
            }
        }
        for (int j = n - 1; j >= 1; j--)
            dp[i][j][0] = max(dp[i][j][0], dp[i][j + 1][0] + a[j][i]);
    }

    out(max(dp[m][n][0], dp[m][n][1]), "\n");
