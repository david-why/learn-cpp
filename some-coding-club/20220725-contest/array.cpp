// https://www.luogu.com.cn/problem/P1438?contestId=76484
// P1438 无聊的数列
#include <bits/stdc++.h>
using namespace std;
...
{
    return Node{a.sum + b.sum, a.l, b.r};
}

void update_tag(Node &node, Tag &tag, const Tag &t)
{
    // if (t.r < node.l || t.l > node.r)
    //     return;
    int L = max(node.l, t.l), l = L - t.l;
    int R = min(node.r, t.r), d = R - L + 1;
    // cout << "--- node{" << node.l << "-"<<node.r<<" "<<node.sum<<"} +tag{"<<t.l <<"-"<<t.r<<" "<<t.d<<"x+"<<t.k<<"}"<<endl;
    node.sum += 1ll * (l + (l + d - 1)) * t.d * d / 2 + 1ll * d * t.k;
    tag.d += t.d;
    tag.k += t.k;
}
...
int main()
{
    ...
    vector<Node> vn(n);
    for (int i = 0; i < n; i++)
        vn[i] = Node{in, i + 1, i + 1};
    st.build(vn);
    cerr << vn[4819].sum << endl;
