// https://www.luogu.com.cn/problem/P1835?contestId=88225
// Luogu P1835 素数密度
#include <bits/stdc++.h>
using namespace std;
...
const int maxt = 50000; // sqrt(1<<31)
Util::Prime<maxt> pr;

int main()
{
    inll(l, r);
    vector<bool> np(r - l + 1);
    for (int p : pr)
        for (ll i = max((l + p - 1) / p, 2ll) * p; i <= r; i += p)
            /*cout << "-- " << p << " " << i << endl, */ np[i - l] = true;
    int ans = 0;
    for (bool b : np)
        ans += !b; //, cout<<(b?1:0);cout<<endl;
    if (l == 1)
        ans--;
    out(ans, "\n");
