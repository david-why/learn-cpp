// https://www.luogu.com.cn/problem/P8563?contestId=88225
// Luogu P8563 Magenta Potion
#include <bits/stdc++.h>
using namespace std;
...
#define frv(n, i) for (int i = 0; i < n; i++)
...
#define gv get_value(ch[li], ch[ri])
    ...
    class SegTree
    {
        static constexpr bool has_tag = !is_same<Tag, Empty>::value;
        GetValue get_value;
...
struct Node
{
    ll v;
    ll lv, rv;
    ll gm() const { return max({lv, rv, v}); }
};

const ll lim = 1ll << 30, too = 1ll << 50;

constexpr ll mks(ll v)
{
    if (v < -lim)
        return -too;
    if (v > lim)
...
int main()
{
    inint(n, q);
    invi(n, v);
    auto st = SegTree::make<Node>(n, [](const Node &a, const Node &b)
                                  {
                                    Node x;
                                    x.v = mks(a.v * b.v);
                                    x.lv = max({1ll, a.lv, mks(a.v * b.lv)});
                                    x.rv = max({1ll, b.rv, mks(a.rv * b.v)});
                                    x.v = max(1ll, mks(a.v * b.v));
                                    return x; });
    vector<Node> vn(n);
    fri(n) vn[i] = {v[i], v[i] < 0 ? 1 : v[i], v[i] < 0 ? 1 : v[i]};
    st.build(vn);
    while (q--)
    {
        inint(op, x, y);
        if (op == 1)
            st.add(x - 1, {y, y < 0 ? 1 : y, y < 0 ? 1 : y});
        else if (op == 2)
        {
            ll v = st.query(x, y).gm();
            if (v == too)
                out("Too large\n");
            else
                out(max(v, 1ll), "\n");
        ...
        {
            Node nd = st.query(x, y);
            cout << nd.v << " " << nd.lv << " " << nd.rv << endl;
        }
    }
