// https://www.luogu.com.cn/problem/P1620?contestId=85850
// Luogu P1620 漂亮字串
#include <bits/stdc++.h>
using namespace std;
...
ll solve(ll cx, ll co, ll mx, ll mo)
{
    if (!mx)
        return min(mo, co);
    if (!mo)
    ...
    {
        if (cx)
            swap(cx, co), swap(mx, mo);
        ans++;
        if (--co)
        {
            ll c = min(co, (m + 1) * (mo - 1));
            co -= c;
            ans += c;
        }
    }
...
{
    ll cx, co, mx, mo;
    while (cin >> cx >> co >> mx >> mo)
        cout << solve(cx, co, mx, mo) << endl;
