// https://www.luogu.com.cn/problem/P1440?contestId=85850
...
namespace Struct
{
    template <typename T, typename Comp = less<T>, typename Container = deque<pair<int, T>>> // default: find max
    class MonoQueue
    {
        // typedef deque<pair<int, T>> Container;
        Container q;
        int n, m, idx;
        Comp comp;

    public:
        MonoQueue() : q(), n(0), m(0), idx(0) {}
        MonoQueue(int n, int m) : q(), n(n), m(m), idx(0) {}
        ...
        T &add(const T &value)
        {
            ...
            idx++;
            if (idx >= m)
            while (!q.empty() && q.front().first < idx - m)
                q.pop_front();
            return q.front().second;
        }
        T &query() { return q.front().second; }
    };
}
using MyIO::in;
using MyIO::out;
...
Struct::MonoQueue<int, greater<>> q;
