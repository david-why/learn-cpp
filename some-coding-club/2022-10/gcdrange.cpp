// https://www.luogu.com.cn/problem/P1890?contestId=85850
// Luogu P1890 gcd区间
#include <bits/stdc++.h>
using namespace std;
...
#define qpow Arithmetic::qpowm<mod>
    ...
    public:
        Combs() : inv(), fac() {}
        Combs(T n) : Combs() { init(n); }
        void init(T n)
        {
            inv.clear(), fac.clear();
            inv.reserve(n), fac.reserve(n);
            inv.pb(1), fac.pb(1);
            for (int i = 1; i < n; i++)
                inv.pb(inv.back() * finv<mod, T>(i) % mod),
...
namespace Struct
{
    ...
    public:
        ...
        MonoQueue(size_t n, size_t m) : q(), n(n), m(m), idx(0) {}
