// https://www.luogu.com.cn/problem/T293496?contestId=92706
// T293496 LCM
#include <bits/stdc++.h>
using namespace std;
...
set<map<int, int>> s;
unordered_set<ll> ss;
unordered_map<int, int> mx, mx2, cnt;
...
int main()
{
    ...
    fri(n)
    {
        const map<int, int> &m = a[i];
        map<int, int> ch;
        ll val = 0;
        for (auto &p : m)
            if (mx[p.first] == p.second && cnt[p.first] == 1)
            val ^= ((p.first*1ll) << p.second%32) * p.first * p.second;
                // ch[p.first] = max(mx2[p.first], p.second);
// out-i-nl;for (auto &p : m)
