// https://www.luogu.com.cn/problem/T293498?contestId=92706
// T293498 最短子串
#include <bits/stdc++.h>
using namespace std;
...
bool check(int k)
{
    ...
    for (int i = 0; i < k; i++)
        win[s[i] - 'A']++;
    // for (int i =0;i<26;i++)out-win[i]-" ";out-nl;
    int cnt = 0;
    for (int i = 0; i < 26; i++)
        if (win[i] >= req[i])
            cnt++;
...
int main()
{
    ...
    for (char c : t)
        req[c - 'A']++;
    // for (int i =0;i<26;i++)out-req[i]-" ";out-nl;
    int lt = 1, rt = 1e5, ans = 1e6;
    while (lt <= rt)
    {
        int md = (rt + lt) / 2;
        if (check(md))
