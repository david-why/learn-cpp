// https://www.luogu.com.cn/problem/P8590?contestId=92706
...
namespace simple_io
{
    ...
    public:
        ...
        input_wrapper &operator()(pair<T *, U> &&val, Args &...args)
        {
            for (size_t i = 0; i < (size_t)val.second; i++)
                is >> val.first[i];
            return operator()(args...);
        }
...
cim = 1e6 + 7, mod = 998244353;
ll psm[maxn], sqr[maxn], a[maxn];
...
int main()
{
    ...
    in - n - k;
    in - mp(a + 1, n);
    fri(n) psm[i] = (psm[i - 1] + a[i] + mod) % mod, sqr[i] = (sqr[i - 1] + a[i] * a[i]) % mod;

    ll ans = sqr[n] * k % mod;
    frv(k, m)
    {
        int ptr = upper_bound(a + 1, a + n + 1, -m / 2) - a;
        ans = ans + 2 * psm[ptr - 1] % mod + ptr - 1;
        ans = ans + 2 * (psm[n] - psm[ptr - 1] + mod) % mod * m % mod + m * 1ll * m % mod * (n - ptr + 1);
        ans %= mod;
    }
    out - ans - nl;
