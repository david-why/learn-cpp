...
#define frv(n, i, x) for (int i = x; i <= n; i++)
...
{
    in(h, n);
    fri(h) fj0(n) res[i] |= ((int)in) << j;
    dp[0][0] = 1;
    fri(h) for (int s = 0; s < (1 << n); s++) if (!(s & (s << 1)) && (s & res[i]) == s) for (int t = 0; t < (1 << n); t++) if (!(s & t)) dp[i][s] += dp[i - 1][t];
    int ans = 0;
    fi0((1 << n)) ans = (ans + dp[h][i]) % mod;
    out - ans - nl;
