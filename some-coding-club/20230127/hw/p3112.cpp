...
namespace simple_io
{
    class input_wrapper
    {
        ...
        struct __sf_helper
        {
            ...
            typename enable_if<!is_integral<Function>::value, input_wrapper &>::type operator+(Function v) { return in + make_pair(n, v); }
            template <typename Function>
            typename enable_if<!is_integral<Function>::value, input_wrapper &>::type operator-(Function v) { return in + make_pair(n, v); }
        };
...
const int maxn = 22, maxs = 1 << 20, maxh = 1e9 + 7;

int n, m;
ll h[maxs];
// dp[S]: S cows, maximum stability
// dp[S]=max{i in S|dp[S-{i}]>=w[i]}(min(dp[S-{i}]-w[i],s[i]))
ll dp[maxs];
struct cow
{
    ll h, w, s;
...
{
    in(n, m);
    in + n - [](int h, int w, int s)
    { cows.pb({h, w, s}); };

    for (int s = 1; s < (1 << n); s++)
        h[s] = h[s - (s & -s)] + cows[(int)log2(s & -s)].h;

    dp[0] = 1e18;
    ll ans = -1e18;
    for (int s = 0; s < (1 << n); s++)
    {
        fi0(n) if ((s & (1 << i)) && (dp[s - (1 << i)] >= cows[i].w))
            dp[s] = max(dp[s], min(dp[s - (1 << i)] - cows[i].w, cows[i].s));
        if (h[s] >= m)
            ans = max(ans, dp[s]);
    }
