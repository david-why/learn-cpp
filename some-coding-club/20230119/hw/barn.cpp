...
namespace simple_io
{
    ...
    public:
        ...
        // input_wrapper &operator+(tuple<Iter, Iter, Function> tp) { return *this + make_tuple(get<0>(tp), get<1>(tp), __util::function_t<Function>(get<2>(tp))); }
...
void dfs(int i, int p, const vector<int> con[])
{
    for (int j : con[i])
        if (j != p)
            dfs(j, i, con);
    ...
    for (int j = 0; j < 3; j++)
    {
        ...
        for (int k : con[i])
            if (k != p)
                f[i][j] = f[i][j] * (f[k][(j + 1) % 3] + f[k][(j + 2) % 3]) % mod;
    }
}
...
int main()
{
    ...
    { tr.add_edge(x, y); } +
        k + [](int b, int x)
    { c[b] = x - 1; };
    tr.run_dp(function<void(int, int, const vector<int> *)>(dfs), 1);
    out - (f[1][0] + f[1][1] + f[1][2]) % mod - nl;
