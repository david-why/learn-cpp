...
namespace tree
{
    ...
    public:
        ...
        template <typename T>
        T run_dp(function<T(int, int, const vector<int>[])> dfs, int root = -1) const { dfs(root < 0 ? rt : root, 0, con); }
        template <typename T>
        T run_dp(function<T(int, int)> dfs, int root = -1) const { dfs(root < 0 ? rt : root, 0); }
        template <typename T>
        T run_dp(function<T(int, int, const tree &)> dfs, int root = -1) const { dfs(root < 0 ? rt : root, 0, *this); }
    };
}
namespace fenwick
{
...
{
    tr.resize(n = in);
    fri(n - 1) tr.add_edge(i + 1, in);
    function<void(int, int, const vector<int>[])> dfs = [&](int i, int p, const vector<int> con[])
    {
        for (int j : con[i])
            if (j != p)
                dfs(j, i, con);
        ...
        for (int j : con[i])
            if (j != p)
                md = max(md, dep[j]);
        // out - mt(i,p,md,md+con[i].size()-1)-nl;
        dep[i] = md + con[i].size() - 1;
    };
    tr.run_dp(dfs);
    out - (dep[1] + 1) - nl;
