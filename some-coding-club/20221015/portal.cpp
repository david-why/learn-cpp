// https://www.luogu.com.cn/problem/P6464?contestId=88000
// Luogu P6464 传送门
#include <bits/stdc++.h>
using namespace std;
...
// vector<vector<int>> dis, bk;

int dis[maxn][maxn], bk[maxn][maxn];
...
int main()
{
    ...
    {
        inint(x, y, w);
        ed.pb(make_tuple(x, y, w));
    }
    // dis.assign(n + 1, vector<int>(n + 1, 1e8));
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= n; j++)
            dis[i][j] = i == j ? 0 : 1e8;
    // for (int i = 1; i <= n; i++)
