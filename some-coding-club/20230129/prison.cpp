...
#define ij2d(i, j) ((i)*m + (j))
#define idx ij2d(i, j)
#define d2i(d) ((d) / m)
#define d2j(d) ((d) % m)
#define dd(i, j) (dis[loc[i]][loc[j]])
...
void floyd()
{
    ...
    for (int i = 0; i < n * n; i++)
    {
        if (blk[i])
            continue;
        if (d2i(i) > 0)
            dis[i][i - m] = blk[i - m] ? 1e9 : 1;
        if (d2i(i) < n - 1)
            dis[i][i + m] = blk[i + m] ? 1e9 : 1;
        if (d2j(i) > 0)
            dis[i][i - 1] = blk[i - 1] ? 1e9 : 1;
        if (d2j(i) < m - 1)
            dis[i][i + 1] = blk[i + 1] ? 1e9 : 1;
    }
    for (int i = 0; i < n * n; i++)
...
bool solve(int v)
{
    ...
    for (int i = 0; i < spc; i++)
    dp[fin][i]=1e9;
    for (int s = 2; s < (1 << spc); s++)
        for (int i = 0; i < spc; i++)
        {
            int p = s&(s-1);
            for (int j = 0; j < spc; j++) if (s&(1<<j))
            {
                if (dp[p-(1<<j)][j]>=dd(i,j))
                dp[s][i]=max(dp[s][i], typ[i]?v:dp[p-(1<<j)][j]-dd(i,j));
            }
        }
    for (int i = 0; i < spc; i++)
    if(dp[fin][i]!=1e9)return true;
    return false;
}
