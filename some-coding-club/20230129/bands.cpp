...
namespace simple_io
{
    class input_wrapper
    {
        ...
        struct __sf_helper
        {
            ...
            __sf_helper operator+(Int v) { return __sf_helper{in, n + v}; }
            template <typename Int>
            typename enable_if<is_integral<Int>::value, __sf_helper>::type operator-(Int v) { return __sf_helper{in, n - v}; }
            template <typename T>
            input_wrapper &operator-(T *v) { return in(make_pair(v, (int)n)); }
            template <typename Function>
            typename enable_if<!is_integral<Function>::value && !is_pointer<Function>::value, input_wrapper &>::type operator-(Function v) { return in + make_pair(n, v); }
        };
...
const int maxn = 1e5 + 7, maxm = 20;

int pre[maxn][maxm], dp[1 << maxm], cnt[maxm], sm[1 << maxm];
int n, m;
...
int main()
{
    ...
    {
        a--, cnt[a]++;
        fj0(m) pre[i][j] = pre[i - 1][j] + (a != j);
        i++;
    };

    for (int s = 1; s < (1 << m); s++)
        sm[s] = sm[s - (s & -s)] + cnt[__builtin_ctz(s)];
    ...
    for (int s = 1; s < (1 << m); s++)
        fi0(m) if (s & (1 << i))
        {
            int l = sm[s - (1 << i)] + 1, r = sm[s];
            dp[s] = min(dp[s], dp[s - (1 << i)] + pre[r][i] - pre[l - 1][i]);
        }
