...
ll c[maxn], n, ans = 1e17;
ll cnt[maxn], prt[maxn];
...
void dfs2(int i, int p, ll tp, ll tc)
{
    for (auto [j, l] : con[i])
    {
        if (j == p)
            continue;
        dfs2(j, i, tp + prt[i] - prt[j] - cnt[j] * l + (tc + cnt[i] - cnt[j]) * l, tc + cnt[i] - cnt[j]);
    }
    ll a = tp + prt[i];
    // cout << i << " " << tp << " " << tc << " " << a << endl;
    if (a < ans)
        ans = a;
    // if (pr + fr[i] < ans)
    //     cout << ans << " " << i << " " << pr << " " << fr[i] << endl, ans = pr + fr[i];
    // return pr;
}
